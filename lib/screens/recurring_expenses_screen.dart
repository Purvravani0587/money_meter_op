...
class _RecurringExpensesScreenState extends State<RecurringExpensesScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    String formatDate(DateTime? dt) {
      if (dt == null) return '';
      return '${dt.day.toString().padLeft(2, '0')}/${dt.month
          .toString()
          .padLeft(2, '0')}/${dt.year}';
    }
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) =>
          AlertDialog(
            ...
            content: Text(
                'Are you sure you want to ${actionLabel.toLowerCase()} "${item
                    .name}"?'),
            actions: [
              TextButton(
            ...
            child: Column(
              ...
              children: [
                ...
                ListTile(
                  ...
                  subtitle: Text(
                      'Date: ${item.date.isNotEmpty
                          ? item.date
                          : "N/A"} • Status: ${item.status.isNotEmpty ? item
                          .status : "Active"}'),
                ),
                const SizedBox(height: 12),
              ],
